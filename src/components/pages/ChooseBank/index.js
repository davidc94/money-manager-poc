...
export default class ChooseBank extends PureComponent {
  ...
  renderListItem = ({ item }) => {
    ...
    return (
      <TouchableOpacity onPress={() => this.selectBank(item.bankName)}
        style={{ flexDirection: 'row', padding: 20, marginTop: 10, marginHorizontal: 10, borderColor: bdColor, borderWidth: 1 }}>
        <View style={{ width: 50, height: 50, margin: 5, flex: 1 }}>
          <Image style={{ resizeMode: 'contain' }} source={item.logoImage} />
        </View>
